...
type DataFromJSON = Array<{ passageRef: string, key: string, offset: number, length: number }>

export default class Bible {
    static getQueryString(biblePassage: string): { youVersionUri: string, queryString: string } | 'invalid' {
        ...
        const temp = [ref.substring(0, ref.lastIndexOf(" ")), ref.substring(ref.lastIndexOf(" ") + 1)];
        ...
        if (colons === 0 && hyphens === 0) {
            ...
            return {
                youVersionUri: `https://www.bible.com/bible/111/${code}.${temp[1].replace(':', '.')}.NIV`,
            ...
            if (block.entityRanges.length > 0) {
                block.entityRanges.forEach(entity => {
                    ...
                    if (data.type === 'LINK' && data.data.url.includes('biblegateway')) {
                        verses.push({
                            passageRef: block.text.slice(entity.offset, entity.offset + entity.length),
                            key: block.key,
